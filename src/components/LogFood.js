...
export default function LogFood() {
  ...
  const fetchDailyData = useCallback(async () => {
    ...
    try {
      ...
      const logRes = await axios.get(`https://calorie-app-backend-d3jb.onrender.com/api/daily-log`, {
        headers: { Authorization: `Bearer ${token}` },
  ...
  const handleSearch = async (e) => {
    ...
    if (term.length >= 1) {
      try {
        const res = await axios.get(`https://calorie-app-backend-d3jb.onrender.com/api/foods?search=${term}`);
        if (res.data.success) {
  ...
  const handleLogFood = async () => {
    ...
    try {
      const res = await axios.post(
        `https://calorie-app-backend-d3jb.onrender.com/api/log-food`,
  ...
  const executeDelete = async (foodEntryId) => {
    ...
    try {
      const res = await axios.delete(
        `https://calorie-app-backend-d3jb.onrender.com/api/daily-log/${dailyLog._id}/foods/${foodEntryId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
